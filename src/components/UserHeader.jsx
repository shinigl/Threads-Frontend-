...
const UserHeader = () => {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      try {
        const res = await fetch(`${apiUrl}/api/users/profile/${username}`, {
          method: "GET",
          credentials: "include", // Corrected: Send jwt cookie
        });
        const data = await res.json();

        if (data.error || data.message) { // Handle 401 or other errors
          toast.error(data.error || data.message, { position: "top-center", autoClose: 500 });
          setTimeout(() => navigate("/error"), 500);
          return;
        }
  ...
  const handleFollowUnfollow = async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/users/follow/${user._id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include", // Add this to send jwt cookie
        body: JSON.stringify({ userId: user._id }),
      });
  ...
  if (loading) return <Loader />;
  if (!user) return <div>Profile not found</div>; // Guard against null user
  ...
  return (
    <>
      ...
      <div className={styles.container}>
        <div className={styles.profile}>
          <div className={styles.info}>
            <h2 className={styles.name}>{user.name || "Unknown"}</h2>
            <p className={styles.username}>@{user.username || "N/A"}</p>
